...
let allCaps = [];
const firstName = runners.map(function(runner) {
  return allCaps.push(toUpper(runner.first_name));
});

function toUpper(runner) {
...
function runnersWithLargeShirts() {
  const largeShirt = runners.filter(runner => runner.shirt_size === "L");

  largeShirts.push(largeShirt);
  return largeShirts;
}

console.log(runnersWithLargeShirts());
// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = [];
const sumOfTickets = runners.reduce(
  (total, runners) => total + runners.donation,
  0
);
ticketPriceTotal.push(sumOfTickets);
console.log(ticketPriceTotal);
// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1. The community gives a special mention to the donors of more than 250.
let greatDonors = [];
greatDonors = runners.filter(runner => runner.donation > 250);
console.log(greatDonors);
